...
`MMb     dMM'        `MM `MM                                `MM'    `MM'          MM       
 ...
 M `Mb d' MM 6M'  `Mb MM  MM 6M'   `Mb `Mb   dMb   d'        MMMMMMMMMM  MM    MM MM'  `Mb 
 M  YM.P  MM MM    MM MM  MM MM     MM  YM. ,PYM. ,P         MM      MM  MM    MM MM    MM 
 M  `Mb'  MM MMMMMMMM MM  MM MM     MM  `Mb d'`Mb d' MMMMMMM MM      MM  MM    MM MM    MM 
 M   YP   MM MM       MM  MM MM     MM   YM,P  YM,P          MM      MM  MM    MM MM    MM 
 M   `'   MM YM    d9 MM  MM YM.   ,M9   `MM'  `MM'          MM      MM  YM.   MM MM.  ,M9 
_M_      _MM_ YMMMM9 _MM__MM_ YMMMMM9     YP    YP          _MM_    _MM_  YMMM9MM_MYMMMM9  

edited: 1/26
developers:
babydev	discord babyydev#1904
...
function Library:create(options)
	...
	if readfile and writefile and isfile then
		if not isfile("MellowSettings.json") then
			writefile("MellowSettings.json", HTTPService:JSONEncode(settings))
		end
		settings = HTTPService:JSONDecode(readfile("MellowSettings.json"))
	...
	if getgenv and getgenv().MellowUI then
		getgenv():MellowUI()
		getgenv().MellowUI = nil
	end
	...
	creditsTab:credit{Name = "babydev", Description = "Hub-Libary Developer", Discord = "babyydev#1904", V3rmillion = "babydev"}
	creditsTab:credit{Name = "Repository", Description = "Hub-Libary", Github="https://github.com/babycodez5k/Mellow-Hub"}
